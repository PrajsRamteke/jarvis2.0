 ...
 * - ~/.jarvis/agent/presets.json (global)
 * - <cwd>/.jarvis/presets.json (project-local)
 *
...
import type { Api, Model } from "@prajwal/jarvis-ai";
import type { ExtensionAPI, ExtensionContext } from "@prajwal/jarvis-coding-agent";
import { DynamicBorder, getAgentDir } from "@prajwal/jarvis-coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@prajwal/jarvis-tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".jarvis", "presets.json");
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify(
				"No presets defined. Add presets to ~/.jarvis/agent/presets.json or .jarvis/presets.json",
				"warning",
			);
			return;
		}
